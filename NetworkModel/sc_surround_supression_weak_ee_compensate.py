...
# In this scipt you can vary recurrent excitatory connectivity (con_ee_factor) and 
# correspoinding synaptic weight (jee_factor)
# Output files name carry the information about variable changed
...
con_ee_factor = [0.4] # Factor by which the connectivity is supressed

jee_factor = [1.0,1.2,1.4,1.6,1.8,2.0]

for a1 in range(len(jee)):
    for a2 in range(len(jie)):
        for a3 in range(len(jei)):
            for a4 in range(len(jee_factor)):
                cee = cee_base*con_ee_factor[0]
                ...
                output_file = 'XCent_Surr_Jee_'+str(jee[a1]) + '_Jei_' + str(jei[a3]) + '_Jie_' + str(jie[a2]) + '_cee_' + str(con_ee_factor[0]) + '_jeefactor_' + str(jee_factor[a4])
                # output_file = 'test'
                np.random.seed(0)
                nest.ResetKernel()
                nest.SetKernelStatus({
                    'local_num_threads': 4,
                    'resolution': 0.1,
                    'data_path': './data_low_recurrence_2_weak_ee',
